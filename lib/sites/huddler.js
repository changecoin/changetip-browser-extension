addModule('huddler', function(module, moduleID) {
	...
	module.init = function() {
		...
		if (isForumPage || isGalleryPage || isComposePage) {
			...
			console.log(document.querySelectorAll(".HM-user-menu"));
			...
			$('article a[href^="/u/"]').each(module.addAuthorTipButton);
		}
	};
	...
	module.getTipMessage = function(tipAmount) {
		return ChangeTip.applyTipTemplate(tipAmount) + ' ' + module.ctMentionString + ' collect it at: ' + module.getOneTimeTipLink();
	};

	module.getOneTimeTipLink = function() {
		return 'https://tip.me/example_one_time_tip_link';
	};
	...
	module.createArticleTipButton = function(userid, username) {
		var tipButton = Utils.createElement('a', null, 'changetip-button changetip-inline-button');
