addModule('twitter', function(module, moduleID) {
	...
	module.attachButtonToTextarea = function(textarea) {
		...
		if (!username) {
			...
			username = $(textarea).closest('.original-tweet-item').find('.username').text();
			avatar = $(textarea).closest('.original-tweet-item').find('img.avatar');
		}
		if (!username) {
			// modal tweets are tricky because content isn't yet loaded...
		...
		tipButton = module.createTipButton(username, avatar);
	...
	module.createTipButton = function(username, avatar) {
	...
	module.submit = function(tipAmount) {
		...
		module.textarea.textContent = text;
	};
