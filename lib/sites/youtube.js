addModule('youtube', function(module, moduleID) {
	...
	module.handleEditorFocus = function(e) {
		var tipButton,
			commentsContainer, username, avatar;

		if (e.type === 'focus') {
			// if this is a top level textarea (meaning no reply button), we can't actually get
			// the username and avatar of OP, so we won't add a changetip button.
			//
			// TODO: this can be done but it's high effort - going to involve message passing
			// from the main page and from this iframe to the extension's background page.
			commentsContainer = $(e.target).closest('.zPa.g9');
			if (!commentsContainer.length) {
				console.log('top level, do nothing');
				return;
			}
