...
function completely(container, config) {
    ...
    config.hintColor =                      config.hintColor;
    config.backgroundColor =                config.backgroundColor || '#fff';
    config.dropDownBorderColor =            config.dropDownBorderColor;
    config.dropDownZIndex =                 config.dropDownZIndex || 100; // to ensure we are in front of everybody
    config.dropDownOnHoverBackgroundColor = config.dropDownOnHoverBackgroundColor || '#ddd';
    ...
    var createDropDownController = function(elem) {
        ...
        var p = {
            ...
            refresh : function(token, array) {
                ...
                // don't return anymore, because we're not using the typeahead hint due to scroll issues.
